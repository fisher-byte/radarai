...
def _build_updates_index(updates: list[dict]) -> str:
...
def sync_updates(project_root: Path, public_root: Path) -> None:
    updates = _read_json(project_root / "data" / "updates.json")
    ...
    _write_text(updates_root / "README.md", _build_updates_index(updates))
...
def main() -> None:
    ...
    print(f"[sync_public_content] ok project_root={project_root}")
